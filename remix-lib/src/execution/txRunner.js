...
class TxRunner {
  ...
  _executeTx (tx, gasPrice, api, promptCb, callback) {
    ...
    if (api.personalMode()) {
      ...
      this._sendTransaction(executionContext.web3().adh.sendTransaction, tx, null, callback)
    }
  }
  ...
  runInNode (from, to, data, value, gasLimit, useCall, confirmCb, gasEstimationForceSend, promptCb, callback) {
    ...
    if (useCall) {
      tx.gas = gasLimit
      return executionContext.web3().adh.call(tx, function (error, result) {
        callback(error, {
          result: result,
          transactionHash: result ? result.transactionHash : null
        })
    ...
    executionContext.web3().adh.estimateGas(tx, function (err, gasEstimation) {
...
async function tryTillReceiptAvailable (txhash, done) {
  return new Promise((resolve, reject) => {
    executionContext.web3().adh.getTransactionReceipt(txhash, async (err, receipt) => {
      if (err || !receipt) {
...
async function tryTillTxAvailable (txhash, done) {
  return new Promise((resolve, reject) => {
    executionContext.web3().adh.getTransaction(txhash, async (err, tx) => {
      if (err || !tx) {
        // Try again with a bit of delay if error or if result still null
        await pause()
