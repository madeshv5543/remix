...
class TxListener {
  constructor (opt) {
    ...
    opt.event.udapp.register('transactionExecuted', (error, from, to, data, lookupOnly, txResult) => {
      ...
      if (!this._isListening) return // we don't listen
      if (this._loopId && executionContext.getProvider() !== 'vm') return // we seems to already listen on a "web3" network
      executionContext.web3().adh.getTransaction(txResult.transactionHash, (error, tx) => {
  ...
  _startListenOnNetwork () {
    this._loopId = setInterval(() => {
      ...
      executionContext.web3().adh.getBlockNumber((error, blockNumber) => {
  ...
  _manageBlock (blockNumber) {
    executionContext.web3().adh.getBlock(blockNumber, true, (error, result) => {
      if (!error) {
  ...
  _resolveTx (tx, receipt, cb) {
    ...
    if (!tx.to || tx.to === '0x0') { // testrpc returns 0x0 in that case
      ...
      if (!contractName) {
        executionContext.web3().adh.getCode(tx.to, (error, code) => {
          if (error) return cb(error)
          if (code) {
